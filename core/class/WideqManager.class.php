...
class WideqManager {
  ...
  private static function getPython(){
    if(self::$pythonBash == null){
      ...
      self::$wideqDir = exec("`cd \"$dir\"; pwd`");
      // python.cmd generated by the install_apt script
      self::$pythonBash = file_get_contents( self::$wideqDir . 'python.cmd');
      if(self::$pythonBash === false){
        LgLog::debug('no file (' . self::$wideqDir . 'python.cmd) found, use default : ' . self::$pythonBash);
    ...
    $file = self::$wideqDir . self::WIDEQ_SCRIPT;
    // ad +x flag and run the server:
    shell_exec(system::getCmdSudo() ." chmod +x $file");
		// $cmd = system::getCmdSudo() . " $file";
    $cmd = system::getCmdSudo() .' source '. self::$wideqDir . '.env/bin/activate; '
