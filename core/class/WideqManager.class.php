...
class WideqManager {
  ...
  private static function getPython(){
    if(self::$pythonBash == null){
      ...
      self::$pythonBash = file_get_contents( dirname(__FILE__) . self::WIDEQ_DIR . 'python.cmd');
      if(self::$pythonBash === false){
        LgLog::debug('no file (' . dirname(__FILE__) . self::WIDEQ_DIR . 'python.cmd) found, use default : ' . self::$pythonBash);
    ...
    $cmd = system::getCmdSudo() .' source '.dirname(__FILE__) . self::WIDEQ_DIR . 'env/bin/activate && '. self::getPython() . $file;
